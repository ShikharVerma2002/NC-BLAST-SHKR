...
- Challonge requests go **only** through the Cloudflare Worker, which holds
  the Challonge API key server-side. The legacy browser-side CORS-proxy
  waterfall (allorigins / corsproxy.io / cors-anywhere) has been removed so
  the API key is never exposed to clients.
- Offline submission queue is an at-least-once delivery outbox for **both**
  Sheets and Challonge submissions; pending items are flushed on app mount
  and on the `online` event.
